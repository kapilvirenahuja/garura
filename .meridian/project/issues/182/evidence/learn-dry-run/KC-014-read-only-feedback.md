...
**Why this matters for audit and replay:** Evidence files in STM represent the record of what happened. If a learning agent modifies them — even to add annotations — it alters the historical record. A future audit or a play replay that reads the same evidence will get different results than the original run, making the system non-reproducible.

**Enforcement:** The agent's tool permissions should be scoped at invocation time. The play passes read permissions for STM evidence paths and write permissions only for the staging area. The agent should not self-grant broader permissions.
