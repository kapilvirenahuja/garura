# Recipe Review: prepare-architecture
...
| F6 | Pre-flight (C1, F6) | ⚠ | **Advisory:** F6 is explicitly a pre-flight failure condition (intent says "the recipe should halt in pre-flight"). Covered by pre-flight hard halt. No SE-* eval needed since domain work never starts. Strict G2 reading expects an eval, but F6's own text mandates pre-flight handling. |
...
The recipe's Step 5 expected return says:
...
**Mismatches:**
- Recipe expects `stm.architecture_brief_path` → doc-builder returns `stm.output.briefs_written[0]`
- Recipe expects `stm.hub_path` → doc-builder returns `stm.output.hub_path`

This affects Step 5 return validation, SE-8 eval (reads `stm.architecture_brief_path`), and Step 8 SCE-3.

**Fix:** Update recipe Step 5 to read `stm.output.briefs_written[0]` for the architecture brief path and `stm.output.hub_path` for the hub path. Update SE-8 and downstream references accordingly.
...
Run `/create-recipe --rebake prepare-architecture` to fix identified gaps.
