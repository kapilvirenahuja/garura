...
| C3 | constraint | structural | Recipe config | Agent contract `config.repo` field + pre-flight gh check |
| C4 | constraint | structural | Recipe logic | Pre-flight bash — `defect_date=$(date +%Y-%m-%d)` |
