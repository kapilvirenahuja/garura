# Verification: /ship High-Order Play

---

## Gates

### G-001: Play Directory Structure
**Mandatory:** Yes
**Check:** Play follows the canonical directory structure:
```
core/components/plays/ship/
  SKILL.md
  reference/
    intent.yaml
  templates/
    ...
    final-report.md
```
**Evidence:** Glob for all files under `core/components/plays/ship/`. Verify each required file exists.
...
**Mandatory:** Yes
**Check:** Steps 1 and 2 invoke existing atomic plays (`commit-code`, `create-pr`) via Skill tool. Steps 3-5 use inline agent delegations (not non-existent atomic plays).
...
**Mandatory:** Yes
**Check:** Play handles these skip conditions:
...
- Metadata (Issue, Play, Step, Created, Status)
- Task List table
- Completed Outputs
- Current Step
...
**Check:** After play creation, `/sync-claude` deploys to `~/.claude/skills/ship/`.
**Evidence:** Verify play appears in `~/.claude/skills/ship/SKILL.md` after sync.

### G-021: Component Documentation Updated
**Mandatory:** No (advisory)
**Check:** `docs/components/plays.md` updated to include `ship` in the play listing.
**Evidence:** Read plays.md, confirm ship play mentioned.
...
Phase 2 (behavioral — after play files exist):
