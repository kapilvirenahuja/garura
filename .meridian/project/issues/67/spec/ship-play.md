...
There is no existing high-order play in the Meridian codebase. All five existing plays (`commit-code`, `create-pr`, `start-feature`, `start-feature-planning`, `discover-product`) are L1. The `/ship` play will be the first L2, establishing the pattern for chaining atomic plays with guardian-gated checkpoints.

The play chains two existing atomic plays (`commit-code`, `create-pr`) with three direct agent delegations (`review-pr`, `merge-pr`, `return-to-main`) to deliver a branch end-to-end. Steps without existing Atomic plays are handled as inline agent delegations within the L2 orchestrator — not as new atomic plays. This avoids creating L1 shells that have no standalone utility.
...
Invoke `commit-code` atomic play via Skill tool.
...
Invoke `create-pr` atomic play via Skill tool.
...
No existing atomic play. Handled as a direct `repo-orchestrator` invocation using the existing `analyze-pr` skill.
...
No existing atomic play. Direct `repo-orchestrator` invocation.
...
- `commit-code` atomic play
- `create-pr` atomic play
...
- `core/components/plays/ship/templates/checkpoint.md`
- `core/components/plays/ship/templates/guardian-decision.md`
- `core/components/plays/ship/templates/final-report.md`

### Deliberate exclusions:
- No `review-pr` atomic play — review is an inline agent delegation (analyze-pr skill exists)
- No `merge-pr` atomic play — merge is a single gh command via agent
- No `return-to-main` atomic play — trivially simple
