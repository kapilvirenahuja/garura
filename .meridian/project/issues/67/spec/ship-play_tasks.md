...
**File:** `core/components/plays/ship/reference/intent.yaml`
...
**Agent:** code-builder sub-agent (largest artifact, high-order play pattern)
**Blocked by:** T1 (references intent.yaml constraint IDs)
**Parallelizable:** No — Wave 2
**Description:** Create the high-order play SKILL.md with:
...
- Full workflow for each step with play context, expected output, guardian evaluation
...
**File:** `core/components/plays/ship/templates/checkpoint.md`
...
**File:** `core/components/plays/ship/templates/guardian-decision.md`
...
**File:** `core/components/plays/ship/templates/final-report.md`
...
### T9: Sync play to deployment
**Agent:** Direct — invoke `/sync-claude`
**Blocked by:** T8
**Parallelizable:** No — Wave 5
**Action:** Run `/sync-claude` to deploy play.
**Verification:** G-020
...
**Action:** Trace through full ship workflow on deployed play:
...
**Action:** Update `docs/components/plays.md` to include `ship` play.
**Verification:** G-021
...
**Blocked by:** T11
**Parallelizable:** No — Wave 7
**Action:** Run ALL gates G-001 through G-021. Generate final evidence at `.claude/specs/ship-play/evidence/final-verification.md`.
...
|------|--------|------------|
| atomic plays may not support auto_approve context from L2 | High | T2 defines how ship_context is passed; L1 checkpoint logic may need minor update (track as discovered task) |
...
| First high-order play — no precedent | Medium | Spec based on ADR 001/003; T10 walkthrough catches misalignment |
