# Recipe Analysis: commit-code (Rebake)
...
Source: `core/components/recipes/commit-code/reference/intent.yaml`
...
## Current Recipe Issues (vs ADR 013 L2)

1. **Stage numbering:** Recipe uses abstract stage numbers (0-7) instead of named phases per ADR 013
2. **Intent-resolver reference:** Recipe references intent-resolver as infrastructure agent — L2 has no runtime intent resolution
3. **DAG references:** Sections on DAG caching and DAG resumption are L4 concepts, not L2
4. **Runtime intent.yaml reading:** Recipe says "read reference/intent.yaml" at startup — L2 recipes are compiled, intent is baked in
5. **Workflow template reference:** Points to old template with Stage 1 (Intent Resolution) — templates have been updated
6. **Eval ownership:** Recipe says "Step evals: Recipe (inline)" but compiled-example.md shows evals baked into each step
