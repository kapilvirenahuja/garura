...
1. **Runtime intent-resolver (Stage 1)** — L2 recipes compile everything into SKILL.md. No runtime DAG resolution. Remove intent-resolver entirely.
...
8. **Extra frontmatter fields** — `model: sonnet` and `allowed-tools:` are skill fields, not recipe fields.
...
**Structure B — Fast execution flow** — This recipe has no checkpoint/brief/approval phase. The only human interaction is inline ambiguity resolution during issue resolution (C5). Structure B fits: Pre-flight → Execution → Evidence.
