# Recipe Analysis: ship (Rebake)

## Current State

L4-era Structure C recipe that chains commit-code → create-pr → merge-pr. All three sub-recipes are now L2 compiled. The parent recipe needs recompilation to L2.

## Semantic Map

### Recipe Structure
- **Workflow:** Structure C (higher-order L2 — chained recipes)
...
| Constraint | Pre-flight | Sub-recipe | Scenario Eval |
|------------|-----------|------------|---------------|
| C1 (branch guard) | Yes | commit-code pre-flight | - |
| C2 (no approvals) | - | Override flag to all sub-recipes | - |
...
| Failure | Sub-recipe Coverage | Scenario Eval |
|---------|-------------------|---------------|
...
- **Workflow:** Structure C (chained L2 recipes)
- **Remove:** intent-resolver, DAG loading/caching, numbered stages
- **Add:** Compiled From, Pause and Resume, Compilation Metadata
- **Preserve:** Approval override pattern (C2), cross-recipe STM data flow, evidence on main
- **No domain agents:** Ship invokes recipes, not agents directly
- **Evals:** Step evals delegated to sub-recipes. Ship owns scenario evals only.
