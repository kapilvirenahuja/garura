...
| E2 | Recipe Execution Engine | Structured Workflows | P1 | Near | E1 | TBD | planned |
...
Without a recipe engine, every AI-assisted workflow is ad-hoc. An engineer asks the AI to "plan a feature" and gets a different process every time — different quality, different artifacts, different levels of completeness. There is no repeatability, no auditability, and no way to enforce quality gates. Teams cannot trust the AI to execute a consistent process.

The Recipe Execution Engine introduces deterministic workflows. After E2 ships, common development operations — feature planning, code review, product discovery, shipping — execute as structured recipes with defined steps, agent delegation, checkpoint approvals (Tether/Vanish), and artifact outputs. L1 recipes handle single-domain tasks (max 2 agent calls). L2 recipes orchestrate multi-domain workflows (max 5 agent calls). Every recipe execution is auditable: you can trace what happened, what was approved, and what artifacts were produced.

The user outcome: an engineer invokes a recipe and gets a reliable, high-quality process every time. The AI follows the recipe, delegates to domain-expert agents, pauses for approvals at the right moments, and produces consistent artifacts. The workflow is no longer dependent on the engineer remembering the right steps.

### Constraints
- In scope: L1 recipe execution (single-domain, max 2 agent calls), L2 recipe orchestration (multi-domain, max 5 agent calls), Tether/Vanish approval protocol at checkpoints, agent delegation framework, checkpoint and evidence artifact production, recipe definition format and loading
- Out of scope: Recipe marketplace or sharing, visual recipe builder, recipe versioning and migration, cross-project recipe inheritance
- Must not break: Direct Claude Code usage without recipes must remain fully functional; users who never invoke recipes must see no behavioral change

### Acceptance Scenarios
- Given a defined L1 recipe, when the user invokes it, then the recipe executes its steps in order, delegates to the correct agent(s), and produces the defined output artifacts
- Given a recipe with a Tether/Vanish checkpoint, when the checkpoint is reached, then execution pauses and waits for explicit user approval before proceeding
- Given a user types "Vanish" at a checkpoint, then the recipe halts cleanly, preserves any completed artifacts, and does not execute subsequent steps
- Given an L2 recipe with agent delegation, when an agent encounters a failure, then the recipe receives a structured failure response and can decide whether to retry, skip, or halt
- Given a recipe execution completes, when the user reviews the session, then checkpoint and evidence artifacts exist at the expected STM paths documenting what happened

### Failure Conditions
- Recipes execute non-deterministically — the same recipe with the same inputs produces different step sequences or artifacts on different runs
- Agent delegation fails silently — a delegated agent errors but the recipe continues as if it succeeded
- Checkpoint approvals are skipped or auto-approved — the Tether/Vanish protocol is not enforced, removing human oversight
- Recipe execution produces no audit trail — no checkpoints, no evidence artifacts, making it impossible to trace what happened
...
### Constraints
- In scope: discover-product recipe (opportunity discovery, vision drafting, vision validation, vision locking), plan-roadmap recipe (epic scoping, brief generation, Tether approval, roadmap drafting, engineering view generation), audience separation between product and engineering artifacts, vision lock protocol
- Out of scope: Backlog management (manage-backlog recipe — separate epic), sprint planning, OKR tracking, financial modeling
...
### Acceptance Scenarios
- Given a problem statement, when the user runs `/discover-product`, then the recipe produces a vision.md with all required sections (problem, users, value prop, strategic goals, metrics, landscape, assumptions, out of scope) and the vision reaches LOCKED status after validation
- Given a locked vision, when the user runs `/plan-roadmap`, then the recipe produces three distinct artifacts: a brief for PM review, a roadmap.md with full IDD epic content, and a roadmap-engineering.md with zero business content
...
- Given a vision is LOCKED, when any recipe attempts to modify vision.md, then the modification is rejected with a structured failure indicating lock status
...
- Vision lock is bypassed — a recipe modifies a LOCKED vision without explicit unlock
...
The user outcome: when a recipe completes, the team knows the output has been checked. Evidence files exist proving what was validated. Quality issues are caught during the workflow, not after deployment.

### Constraints
- In scope: quality-validator agent implementation, verification scenario execution framework, evidence artifact production and storage, quality gate definitions for existing artifact types (vision, roadmap, code), integration with recipe checkpoint system
...
- Must not break: Recipes that predate the quality layer must continue to function without mandatory quality gates; quality validation must be opt-in at the recipe level

### Acceptance Scenarios
- Given a recipe with a quality gate defined, when the recipe reaches the quality checkpoint, then the quality-validator agent executes all defined verification scenarios and produces an evidence artifact
- Given a verification scenario fails, when the quality-validator reports results, then the recipe receives a structured failure with the specific scenario that failed and why, enabling targeted fix
- Given a completed recipe execution, when the user checks the evidence path, then evidence artifacts exist documenting every verification scenario that was executed, with pass/fail status and timestamps
- Given a recipe without quality gates defined, when the recipe executes, then it completes normally without invoking the quality-validator — quality is opt-in, not mandatory
...
The Feature Development Track delivers the complete end-to-end pipeline: issue creation, branch setup, technical planning (spec + design), implementation, verification, and shipping. Each step is a recipe invocation that builds on the prior step's artifacts. The AI assistant has full context at every stage because E1 (Persistent Context) ensures continuity, E2 (Recipe Engine) ensures structure, and E3 (Quality Layer) ensures verification happens before shipping.
...
### Constraints
- In scope: start-feature-planning recipe (spec + design from issue), implement recipe (code from design), ship recipe (PR creation, verification, merge), end-to-end artifact traceability (issue -> spec -> design -> code -> PR), branch management integration via repo-orchestrator
...
- Must not break: Manual feature development workflows must remain functional; engineers who prefer to code without recipes must not be blocked

### Acceptance Scenarios
- Given an issue number, when the user runs `/start-feature-planning`, then the recipe produces a spec and design artifact linked to the issue, stored at the correct STM path
...
- Given completed implementation, when the user runs `/ship`, then the recipe creates a PR, runs verification, and produces evidence artifacts documenting what was checked before merge
...
A powerful system that nobody can set up is a failed system. Today, adopting Meridian OS requires reading documentation, manually creating directory structures, populating CLAUDE.md, understanding the recipe system, and configuring memory — a process that takes hours and requires significant prior context. This is acceptable for the core team building the system, but it is fatal for adoption.

The Adoption & Onboarding epic delivers a setup experience that gets a new team from zero to productive in under 30 minutes. A project initialization recipe handles directory creation, CLAUDE.md population, starter templates, and validation. Progressive enrichment ensures teams start lightweight — no big-bang documentation upfront — and gain depth as they use the system. Setup validation confirms everything is working before the team starts their first real workflow.

The user outcome: a new team runs a single initialization command, answers a few questions about their project, and gets a working Meridian OS setup. They can immediately invoke their first recipe. The 30-minute promise is met.

### Constraints
- In scope: Project initialization recipe (`/init` or equivalent), directory structure creation, CLAUDE.md starter population, progressive enrichment templates, setup validation (confirm working state), first-run guided experience
...
- Must not break: Existing projects that were set up manually must not require re-initialization; the init recipe must detect and respect existing configurations

### Acceptance Scenarios
- Given a new project with no Meridian OS configuration, when the user runs the initialization recipe, then the recipe creates all required directory structures, populates CLAUDE.md with project-relevant context, and produces starter templates — all within 30 minutes of wall-clock time
- Given initialization is complete, when the user runs setup validation, then the validation confirms all required components are in place and the first recipe can be invoked successfully
- Given an existing project with manual Meridian OS configuration, when the user runs the initialization recipe, then the recipe detects existing configuration, preserves it, and only fills gaps — no overwrites
- Given a newly initialized project, when the user invokes their first recipe (e.g., `/discover-product`), then the recipe executes successfully using the initialized context without additional manual setup

### Failure Conditions
- Initialization takes longer than 30 minutes — the zero-friction promise is broken
- The init recipe produces a broken state — setup validation fails, or the first recipe invocation errors due to missing configuration
