...
| `standards_set` | object | `{kb, ltm, stm}` paths resolved from `.meridian/core/config.yaml` `standards_order` |
| `severity_taxonomy_path` | string | Resolved from `.meridian/core/config.yaml` `standards.pr-severity-taxonomy` |
| `intent_summary` | string | 1–3 sentences from `spec/intent.yaml` or PR body |
| `output_path` | string | Where to write `findings.yaml` |

## Resolution

The caller (review-pr Step 2 via `quality-auditor`) MUST resolve all paths from `.meridian/core/config.yaml` — never hardcode. Pre-flight in the play validates `severity_taxonomy_path` exists before invoking the skill.
