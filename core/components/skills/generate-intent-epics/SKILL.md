...
- `epics.md` Rule 1 — the `intent` must name a user and observable outcome, not a layer
...
2. For each capability tagged `component`:
   ...
   - Merge the component's content into the parent's buckets:
     ...
     - Component's `failure_scenarios` → appended to parent's failure scenarios.
...
- `success_scenarios`: map the enriched Success Criteria to scenario entries. Each must have a `scenario` and `evidence` sub-field. At least 2 entries. Every `then` clause must use observable-outcome language. Forbidden words in `then`: `should`, `smooth`, `intuitive`, `seamless`, `better`, `user-friendly`, `fast` (without a number), `improved` (without a measurement).
...
| `D-gie-004` | `success-scenario-phrasing` | How each enriched Success Criterion is mapped to observable-outcome language in each success_scenario `then` clause (Step 3) |
...
| `D-gie-006` | `component-to-parent-merge` | For each component capability, which parent vertical it merges into and which `in_scope` items, rules, and failure scenarios are absorbed (Step 2b) |
...
decisions:
  ...
  # ... one entry per decision listed above (repeat per capability for D-gie-001 through D-gie-005)
```
...
- NEVER use "should" language in success scenario `then` clauses. Observable outcomes only.
- NEVER use a subsystem / component name as the grammatical actor of an `intent` field. The intent subject must be a named persona or a canonical user role per `rules/epics.md` Rule 1. If the source capability reads as a component, it should have been merged in Step 2b, not generated as its own epic.
