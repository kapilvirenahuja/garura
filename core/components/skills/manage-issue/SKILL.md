---
name: manage-issue
description: Read, create, or resolve GitHub issues with optional sub-issue attachment
...
Read existing issues, create new ones, or resolve an issue reference (find existing or create new). Supports attaching issues as sub-issues to a parent.
...
Receive from agent:
- `action` — `read` | `create` | `resolve_or_create`
- `issue_number` — (required for `read`, optional for `resolve_or_create`)
