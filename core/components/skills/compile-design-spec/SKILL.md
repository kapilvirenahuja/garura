...
description: Consolidate personas, screens, flows, wireframes, and interaction patterns into a single `design-spec.md` at `{product_base}experience/design-spec.md`. The consolidated spec is the handoff deliverable for arch and implementation.
...
Called by `designer` during `design` Stage 6. Produces the final consolidated design specification.

## Purpose

A single readable document that ties the UX design together: who the users are, what screens they interact with, how they move through the product, how each screen is laid out, what interaction patterns apply, and what accessibility requirements bind the implementation. The arch play reads this file. The implement play reads this file. A developer onboarding to the project reads this file.
...
- `design_system_path` (path, required) — {product_base}experience/design-system.md. The DS is a sibling artifact authored by draft-design-system in /design Step 5b. Its content is referenced in the final design spec's Visual Identity section and Handoff Notes. Missing this path is a contract failure — the spec cannot be compiled without the DS reference.
...
- Read `design_system_path` and extract the Palette, Typography, Tokens, and Inspirations section headers for use in the Visual Identity section of the compiled spec. Do NOT copy specific hex codes or pixel values into the spec body — the spec references the DS by path only.
...
**Generated by:** design Stage 6
**Generated at:** {ISO-8601}
**Sources:** personas.md, screens/*.md, flows/*.md, scope.yaml, quality-profile.yaml, design-system.md
...
The Design System artifact (authored by draft-design-system in /design Step 5b) defines the visual surface for this product. Referenced sections: Palette, Typography, Tokens, Inspirations. This spec does not reproduce token values or pixel specs — consult the DS directly for implementation of visual aesthetics.

## 8. Handoff Notes

- **arch** reads this file to drive frontend architecture decisions (routing, state management, component library, API surface).
- **implement** reads specific screen files (not the consolidated spec) for per-epic implementation context.
- **Visual design** is scoped via the sibling Design System artifact at {product_base}experience/design-system.md (authored by draft-design-system in /design Step 5b). The design spec references the DS by path and summary — it does NOT inline token values or pixel specs. Screens remain structural wireframes; the DS is the visual surface.
