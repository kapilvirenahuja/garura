...
- **Pattern:** Three-layer hierarchy (Plays -> Agents -> Skills) with Four Crafts separation (Intent, Prompt, Context, Spec) and JSON Contract data flow
- **Components:** Plays (12 defined), Agents (8 defined: 6 implemented, 2 planned), Skills (30 defined), Dual Memory (LTM project-scoped + STM issue-scoped)
- **Data flow:** Play creates JSON contract -> Agent receives contract, performs Context Crafting (discovers LTM, reads STM) -> Skill receives assembled inputs, produces STM artifacts -> Agent enriches contract -> Play validates
- **Key invariant:** Plays never execute domain work directly; all domain operations delegate to agents. Plays own only orchestration, checkpoints, and evidence.
...
- Plays are contracts, never forked -- prescribed step order is non-negotiable
- Agent-first: plays forbidden from using Bash, Grep, Glob, or direct git/gh commands
- Atomic plays: max 2 agent calls; high-order plays: max 5 agent calls (ideal 3)
...
| E2 -- Play Engine | L1/L2 execution, agent delegation, Tether/Vanish protocol | planned |
...
- Intent Primacy philosophy: plays are scaffolding toward future intent-driven autonomous execution where constraints replace prescribed steps
