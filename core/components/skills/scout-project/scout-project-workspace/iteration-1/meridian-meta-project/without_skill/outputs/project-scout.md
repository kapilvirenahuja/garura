...
- **Recipes** (L1/L2): Workflow orchestration. L1 recipes are atomic (human or model invocable, max 2 agent calls). L2 recipes are composite (human-only, chain L1s, max 5 agent calls). 11 recipes defined: commit-code, create-pr, start-feature, plan-roadmap, ship, and others.
- **Agents** (8 defined): Domain experts invoked by recipes via the Task tool. Includes repo-orchestrator, project-orchestrator, code-builder, tech-designer, product-strategist, intent-crafter, intent-resolver, and doc-builder. Agents discover and invoke skills.
...
Data flow: L2 Recipe chains L1s, L1 invokes agents, agents invoke skills, skills produce artifacts to STM.
...
- **Language**: Pure Markdown definitions for agents, skills, recipes, and memory -- no application code
...
1. **Agent-first delegation**: Recipes must never use tools directly when an agent covers that domain. Direct git commands are forbidden inside recipes.
2. **Recipe budget limits**: L1 max 2 agent calls; L2 max 5 (ideal 3).
...
- **JSON contract flow**: Recipe passes contract to agent, agent to skill, skill returns artifact, flows back up.
