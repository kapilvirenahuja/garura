...
| `architecture.md` | Build-arch rules — decisions cite drivers (epics / quality attributes / constraints), tech-agnostic logical layer, source-type discipline, refined-QP-describes / physical-delivers NFR split, industry-documented patterns in tech-stack, no-cycles, end-to-end traceability through layers, systems inventory grounding, security-ratchet-up | `derive-systems-inventory`, `refine-quality-profile`, `derive-logical-architecture`, `derive-physical-architecture`, `derive-tech-stack`, `derive-technical-risks`, `validate-architecture-spec`, `tech-architect`, `tech-designer` |
