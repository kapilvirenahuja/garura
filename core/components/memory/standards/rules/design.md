...
Canonical rules governing design-exp output — personas, screen inventory, user flows, wireframes, and the design spec. Every skill in the `/design-exp` pipeline loads this file.
...
**Enforcement:** `validate-personas` (or the generate-exp validator pass) rejects personas without a job story. SE-2 on design-exp SKILL.md enforces this.
...
**Enforcement:** `validate-personas` (or equivalent). SE-3 on design-exp SKILL.md enforces this.
...
**Enforcement:** `validate-screen-coverage` check category `state_count_below_min`. SE-6 on design-exp SKILL.md enforces this.
...
**Enforcement:** `validate-screen-coverage` check `capabilities_covered == capabilities_total`. SE-5 on design-exp SKILL.md enforces this.
...
**Enforcement:** `map-user-flows` validator pass — `success_scenarios_with_flow == success_scenarios_total` and `failure_scenarios_with_recovery == failure_scenarios_total`. SE-9 and SE-10 on design-exp SKILL.md enforce this.
...
**Enforcement:** `generate-wireframes` + `validate-wireframes` grep blacklist. SE-11 and SE-12 on design-exp SKILL.md enforce this.
...
**Enforcement:** `compile-design-spec` validator greps for hex codes, `rgb(`, `px` spacing, font-family declarations. SE-16 on design-exp SKILL.md enforces this.
...
**Enforcement:** `validate-screen-coverage` or `compile-design-spec`. SE-13 and SCE-5 on design-exp SKILL.md enforce this.
...
**Enforcement:** `compile-design-spec` pattern-table check. SCE-6 on design-exp SKILL.md enforces this.
