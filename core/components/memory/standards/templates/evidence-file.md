...
Consumers: `specify-product`, `design-exp`, `build-arch`, `start-feature-planning`, `implement-epic`, every play that closes with an evidence step.
...
{Only when the play had a pre-lock resolution gate (e.g. specify-product).
...
artifacts. For specify-product: "design-exp reads enriched-capabilities.yaml
and epics/; build-arch reads quality-profile.yaml and epics/". For
start-feature-planning: "implement-epic reads planning/spec.md,
verify.md, tasks.md".}
```
