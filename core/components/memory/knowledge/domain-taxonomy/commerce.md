...
**Signals:**
Any e-commerce or marketplace product. PP-3 = 5 (multi-sided platform) needs multi-seller catalog. PP-4 >= 3 (multi-country) needs localized product information. PP-2 >= 3 needs rich product presentation. BRD keywords: "products", "catalog", "listings", "SKU", "inventory", "categories".
...
**Signals:**
Any multi-item purchase flow. PP-1 >= 4 (consumer) needs guest cart and abandonment recovery. PP-6 >= 3 (market-ready) should have persistent carts. PP-3 >= 4 (enterprise customers) may need multi-cart and approval flows. BRD keywords: "cart", "basket", "add to cart", "checkout", "save for later".
...
**Signals:**
Any product that takes orders. PP-5 >= 3 (bidirectional integration) suggests ERP/shipping integration. PP-3 = 5 (marketplace) needs per-seller order management. PP-6 >= 3 needs proper order lifecycle beyond basic. BRD keywords: "order", "fulfillment", "shipping", "tracking", "order status", "delivery".
...
**Signals:**
Any physical goods commerce. NFR-3 (Performance) >= 3 needs real-time stock validation to prevent overselling under load. PP-4 >= 3 with physical goods needs multi-warehouse consideration. PP-3 = 5 (marketplace) needs per-seller inventory. BRD keywords: "inventory", "stock", "availability", "warehouse", "supply chain".
...
**Signals:**
Consumer e-commerce where social proof matters. PP-1 >= 3 (not purely technical users). PP-3 = 5 (marketplace) needs seller reviews alongside product reviews. PP-2 >= 3 expects polished review presentation. PP-7 = 2 (consumer/lifestyle) heavily benefits from reviews. BRD keywords: "reviews", "ratings", "social proof", "user feedback", "testimonials".
...
**Signals:**
Commerce products using pricing as acquisition/retention lever. PP-6 >= 3 (market-ready) should have at least standard promotions. PP-1 >= 4 (consumer) expects promotional pricing. PP-3 = 5 (marketplace) needs per-seller promotion capabilities. BRD keywords: "promotions", "discounts", "coupons", "sale", "deals", "loyalty", "rewards".
...
**Signals:**
Consumer e-commerce with browsing-heavy behavior. PP-1 >= 4 (consumer) expects wishlist functionality. PP-2 >= 3 (polished UX) should include save-for-later. PP-6 = 1-2 (POC/MVP) can defer wishlists. BRD keywords: "wishlist", "save for later", "favorites", "bookmarks", "gift registry".
...
**Signals:**
Physical goods commerce. PP-7 >= 3 has regulatory return obligations (EU consumer protection). PP-4 >= 3 (multi-country) adds cross-border return complexity. PP-3 = 5 (marketplace) needs seller-level return handling. BRD keywords: "returns", "refund", "exchange", "return policy", "reverse logistics", "RMA".
