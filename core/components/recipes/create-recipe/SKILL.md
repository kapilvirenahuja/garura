...
|----------|-----------|----------------------|
| `pre-flight` | Environmental precondition checkable before domain work | Pre-flight check in compiled recipe |
| `artifact-verifiable` | Observable property of a skill's output artifact | Eval (SE-n) generated by evals-creator |
| `structural` | Process/architectural rule about the recipe itself | Recipe structure (agent boundary table, compilation rules) |
...
- **Brief before checkpoint (Structure A):** Every human checkpoint MUST be preceded by a `doc-builder` brief generation step. The pattern is: domain agents produce artifacts → `doc-builder` generates HTML briefs under `{artifact_base}/briefs/` and regenerates hub.html → recipe presents brief for human review (Tether/Vanish/Orbit). `doc-builder` is a utility agent — it does NOT count against the ≤5 domain agent budget. If the recipe has no brief generation skill for its domain, the compiler must flag this as a gap and create one via `/skill-creator`. The doc-builder contract uses `briefs_requested` (list of artifact names) and `artifact_base` — doc-builder owns output path computation and hub.html lifecycle.
...
Write coverage matrix to `{stm_base}/{issue}/evidence/create-recipe/{recipe-name}/coverage-matrix.md`.
