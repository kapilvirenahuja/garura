...
Intent defines constraints (C1-C17), failure conditions (F1-F12), and scenarios (S1-S10). All 12 FCs (F1-F12) and 10 scenarios (S1-S10) have eval coverage.
...
The judge reads product.yaml and extracts every discrete element: strategic goals (all SG-IDs), success_metrics, target_users, assumptions, out_of_scope, scope.in_scope items, and profiles (if present). Then reads epics.yaml and checks that every input element is addressed:
...
- **orphaned_scope**: a product.yaml scope.in_scope item has no owning epic — it does not appear in any epic's in_scope field (C16)
- **deferred_violation**: an epic's in_scope contains a capability that appears in product.yaml scope.out_of_scope or is marked as deferred (C17)
...
**On orphaned_scope (C16):** If any in_scope item from product.yaml has no owning epic, the orchestrator presents the orphaned items and halts — the scoping agent must create or amend epics to claim ownership. This is a hard halt.

**On deferred_violation (C17):** If any epic claims a deferred or out_of_scope capability, the orchestrator presents the violations and halts — the epic must be corrected to remove the deferred capability. This is a hard halt.
...
- **SE-22 (C16, F11):** Read `stm.coverage_check_path` — verify every item from product.yaml `scope.in_scope` has status `covered` (mapped to at least one epic's `in_scope`). PASS if no in_scope item has status `orphaned_scope`. FAIL if any in_scope capability from product.yaml has no owning epic — it was silently absorbed or dropped without explicit ownership. Halt on FAIL.
- **SE-23 (C17, F12):** Read `stm.coverage_check_path` — verify no epic's `in_scope` contains any capability from product.yaml `scope.out_of_scope` or deferred items. PASS if no `deferred_violation` entries exist. FAIL if any epic claims a capability that the product vision explicitly excludes or defers. Halt on FAIL.
...
- **SCE-9 (S10 — Product Manager):** Read `stm.coverage_check_path`. Verify: (a) every item from product.yaml `scope.in_scope` has an entry with status `covered` (mapped to at least one epic's in_scope), (b) no entry has status `deferred_violation` (epic claiming a deferred/out_of_scope capability), (c) any cross-cutting capabilities that required horizontal epic exceptions have `cross_cutting_justification` documented in the corresponding epic. PASS if full scope-item traceability exists from product.yaml to epic ownership with no orphaned or violated items. FAIL if any in_scope item is orphaned, any deferred item appears in an epic's in_scope, or cross-cutting justifications are missing.
