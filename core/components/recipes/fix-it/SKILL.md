...
To modify this recipe, update intent.yaml and re-run `/create-recipe --rebake fix-it`.
...
```json
{
  "intent_path": "core/components/recipes/fix-it/reference/intent.yaml",
  "stm_base": "{stm_base}",
  "stm": {
    "input": {},
    "output": {
...
**This is the ONLY approval gate in the recipe (C6).**
...
**Approval override (C7, C9):** Pass `approval_override: "auto-proceed"` to suppress all human checkpoints in the ship sub-recipe. After the checkpoint was approved via Tether in Step 5, all remaining steps execute without any further human approval.
