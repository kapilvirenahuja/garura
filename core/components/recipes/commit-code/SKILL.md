...
To modify this recipe, update intent.yaml and re-run `/create-recipe --bake commit-code`.
...
```json
{
  "intent_path": "core/components/recipes/commit-code/reference/intent.yaml",
  "stm_base": "{stm_base}",
  "stm": {
    "input": {},
    "output": {
...
The skip condition implements C9: human approval is required when any mapping has less than high confidence. When all mappings are high confidence, the recipe proceeds autonomously.
