...
defines constraints (C1–C18) and failure conditions (F1–F14); the expectation defines
...
code:
    ...
    {
      "task":   "distill the approved-bound spec: a crisp ICE boundary doc (~1-2 pages) — rules, patterns, designs, config, existing-code map — referencing the epic/ICE/lenses, never copying code",
      "inputs": { "epic_file": "<epic_file>", "functionality_ices": ["<ice paths>"],
...
Gate it mechanically — the boundary doc must stay crisp and code-free:
...
(Step 4 gate), the spec is re-authored and re-gated (`check_spec.py`), and it is
re-presented — the build never starts on an unapproved spec. The box only widens where the
human draws the wall (REC3).
