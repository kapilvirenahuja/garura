#!/usr/bin/env python3
"""
apply_quality.py — persist /quality's quality lens, on a fixed allowlist.
...
Layer rule: pure file writes from disk inputs; no git/gh/network.
    ...
    python3 apply_quality.py --draft <draft_dir> --product-base <product_base> \
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Persist /quality on a fixed allowlist.")
    ...
    if not os.path.isdir(src_root):
        sys.stderr.write(f"apply_quality.py: no draft tree at {src_root}\n")
        sys.exit(2)
