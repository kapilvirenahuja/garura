#!/usr/bin/env python3
"""
validate_quality.py — assert /quality's draft is grounded and considers the slice's hub.

Run over the draft before the checkpoint. In the spine+grounding model the quality lens is an
MD grounding doc (`quality.md`); its SHAPE (the "Is it an agent?", "Load weights", "Controls"
...
carries, which the prose can't enforce:
  ...
  - grounded: every grounding entry names a real source from the slice's HUB (a functionality,
    persona, or journey) — never another lens (the quality read is hub-only).
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Validate /quality's draft grounding + coverage.")
