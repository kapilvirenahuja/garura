...
Intent defines constraints (C1–C10) and failure conditions (F1–F7); the
...
candidate sets, the operator profile; step and scenario eval results SE-1…SE-6 /
...
folder (`${product_base}_evidence/next/`), the durable record.

```bash
rm -rf "${working}"            # working = ${product_base}_status/next/
```
...
| Field | Value |
|-------|-------|
| fingerprint | sha256:551219cc3280366f3cc28de634100178577673ac2176963e63bb93a3911a125d (of `reference/ice.md`) |
...
| step_evals | 6 (SE-1…SE-6; every failure condition covered: F1→SE-5, F2→SE-2, F3→SE-5, F4→SE-3+SE-5, F5→SE-4+SE-5, F6→SE-2+SE-5, F7→SE-6) |
