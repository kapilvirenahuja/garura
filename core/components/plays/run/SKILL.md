...
name: run
position: none
description: 'Write a SLICE''s run lens — how it is deployed and runs: the environments it moves through, the rollout (flags + strategy), the migrations stance, the config/secrets stance, the CI/CD pipeline, and a run target for every architecture component. Reads the slice''s hub + its ARCHITECTURE lens (run deploys arch''s parts) + the KB, grounding every operational choice in a best-fit architecture/technology learning — never the model''s taste; uncovered choices become recorded KB-learning-gap proposals. The fifth and LAST realize play in the ProductOS command model (quality → ux → agentic → arch → run). As the last lens it also runs the lines-up check (all five lenses present + cross-references resolve) and, only then, stamps the slice record status: realized — the marker /grill checks. Writes the run lens, decisions, and (on lines-up) the slice stamp; opens no delivery issue.'
user-invocable: true
---
...
The run lens is six things and only six: the **environments** the slice moves through (dev →
...
gates → deploy on green); and the **targets** — for every architecture component, where and how
it runs. /run **deploys what /arch designed**: every run target binds to a real component in the
slice's architecture lens. It is the one realize lens that reads another (the architecture
lens), because you cannot say how something runs without the parts that run.
...
|-------|--------|-------------------|--------|
| `product-os-keeper` | Search the KB's architecture/technology shelves for the patterns that fit this product, draft the run lens (environments, rollout, migrations, config/secrets, CI/CD, and a target per architecture component) grounded in those learnings, raise a KB-learning-gap proposal for any uncovered choice, and record material choices as decisions | `kb-search`, `author-run-lens` | Draft |
...
than guessing, and grounds every choice and every target in the manifest:
    ...
    {
      "task":    "search the KB's architecture/technology shelves for the patterns that fit this product's conditions; draft this slice's run lens — environments, rollout (flags + strategy), migrations, config/secrets, CI/CD, and a run target for every architecture component — grounding every operational choice in a matched KB learning (or a recorded KB-learning-gap proposal — a candidate architecture/technology learning — for a gap), binding every target to a real architecture component; record the rollout/migration/environment choices as decisions",
...
**SE-3 (F3/C3):** the lens is the six blocks only — `content` has exactly
environments/rollout/migrations/config_secrets/cicd/targets, each non-empty; no other-lens
content.
...
On any GAP, apply REC3/REC5/REC6/REC7/REC11 and re-run before the checkpoint.
...
a run target + no dangling target) ⇒ the run **will** stamp the slice `realized` at Step 6.
`lines_up: false` ⇒ it will **not** stamp; the missing lens / dangling reference is reported and
carried to the checkpoint.
...
migrations stance, the config/secrets stance, the CI/CD pipeline, and the per-component targets —
each grounded in the KB learning (or proposal) it came from, plus any decision, plus the
**lines-up result** and whether the run **will stamp** the slice `realized`. This checkpoint is
...
- **SCE-7 (S7 — reviewer, the checkpoint):** the checkpoint showed the six blocks, the
  groundings, the lines-up verdict, and the stamp intent inline before any write.
...
whether the slice was stamped; step and scenario eval results SE-1…SE-12 / SCE-1…SCE-7;
...
from the task DAG, the Artifacts Produced table (the six run blocks, the targets, the decisions,
any proposals), Next Steps (if the slice was stamped `realized`, run /grill to cut delivery work;
...
| Field | Value |
|-------|-------|
| fingerprint | sha256:b8e473771c7863471443333c01681de99b5da8819ee038d7a36ab8f3b6e8655f (of `reference/ice.md`) |
...
| step_evals | 12 (SE-1…SE-12; SE-5 split a/b, SE-11 split a; one per failure condition) |
