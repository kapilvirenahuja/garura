...
estimate for each. It writes that plan onto each slice — `order`, `effort`, resolved
`depends_on`, and the `status` flip to `planned` — and touches nothing else. One run
...
(order/effort/depends_on/status) on a slice; editing a slice's composition; persisting by
any route other than `scripts/apply_roadmap.py`; persisting before Step 4 approval;
touching ICE, structure, the profile, the lenses, decisions, or the `_deferred` bucket
(C2).
...
|-------|--------|------------------|--------|
| `product-os-keeper` | Read /shape's slices and judge the plan inputs — estimate effort per slice, resolve dependency_notes (+ shared functionalities + their ICE depends_on) into concrete depends_on, propose a value order | `author-roadmap` | Draft |
...
Take the pre-apply "before" picture — every slice across every shaped domain (id,
domain_ref, bundled functionalities, dependency_notes, current plan fields) and a
path→hash census of every file under product-os. Gated step, so a resume preserves it:
...
`plan-draft.yaml` to STM:
    ...
    {
      "task":    "for every slice in the snapshot, estimate effort, resolve depends_on (from dependency_notes + shared functionalities + their ICE depends_on), and propose a value order; draft only",
      "inputs":  { "snapshot_path": "<working>/snapshot.json",
                   "product_base":  "<product_base>" },
      "outputs": { "plan_draft":    "<working>/plan-draft.yaml" }
    }

The skill reads slices + ICE **read-only** and writes only the draft. It assigns no
...
Persist on the fixed allowlist. `apply_roadmap.py` is handed only the plan + the snapshot;
it does a read-modify-write that changes only `order`, `effort`, `depends_on`, and
`status` on each planned slice, preserving every other field, and cannot reach a slice's
...
**SE-1 (F1/C1):** the apply manifest's `changes` are all on slice files — no non-slice
artifact received a plan.
**SE-2 (F2/C2):** each change touches only the plan fields (order/effort/depends_on/
status) — no slice composition, ICE, or other artifact appears.
...
**SE-6 (F6/C6):** non-destructive — every file under product-os is byte-identical to the
snapshot except slice files whose only changed fields are the plan fields; no file added
or removed, no non-slice file changed.
**SE-7 (F7/C7):** every slice still validates against slice v1 — required fields present,
order an integer, effort non-empty when planned.
...
- **SCE-1 (S1 — planner, first plan):** every non-deferred slice has an integer order and
  a non-empty effort; orders are distinct 1..N; every other field and file is byte-
  identical; slices validate v1.
...
| F2 | a write touched a slice's composition or another artifact | revert the out-of-scope write; /roadmap writes only order/effort/depends_on/status on slices | autonomous |
...
| F6 | a non-plan slice field changed, or a non-slice file changed | restore the changed content and re-apply only the plan fields, after a human confirms the restore | human |
| F7 | a written slice fails slice v1 schema validation | re-emit the failing slice to conform before the play completes | autonomous |
