#!/usr/bin/env python3
"""
apply_arch.py — persist /arch's architecture lens, on a fixed allowlist.
...
Layer rule: pure file writes from disk inputs; no git/gh/network.
    ...
    python3 apply_arch.py --draft <draft_dir> --product-base <product_base> \
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Persist /arch on a fixed allowlist.")
    ...
    if not os.path.isdir(src_root):
        sys.stderr.write(f"apply_arch.py: no draft tree at {src_root}\n")
        sys.exit(2)
