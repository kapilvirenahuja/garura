#!/usr/bin/env python3
"""
apply_agentic.py — persist /ux's agentic lens, on a fixed allowlist.

Run only AFTER the human approves the checkpoint. It writes exactly two kinds of thing
and NOTHING else:

  1. The agentic lens — `lens/agentic.yaml` for the capability, written from the draft. This is the
     re-derive, so it overwrites a prior agentic lens (C9 allows the lens itself to change on a
     re-run).
...
def main(argv=None):
    ...
    for dirpath, _dirs, files in os.walk(src_root):
        ...
        for fn in files:
            ...
            is_agentic_lens = (len(parts) >= 2 and parts[-2] == "lens" and parts[-1] == "agentic.yaml")
            is_decision = ("decisions" in parts and fn.endswith(".yaml"))
            if not (is_agentic_lens or is_decision):
