#!/usr/bin/env python3
"""
check_agentic.py — assert /ux's persisted result obeys its guarantees.
...
resume can never compare post-apply against post-apply).
  ...
  - C9/F9  non-destructive: every file in the capability folder is byte-identical to its
           pre-apply snapshot EXCEPT `lens/agentic.yaml` (the re-derive); the profile is
           byte-identical; an accepted decision present before apply is unchanged (never
           edited in place).
  - C2/F2  scope: nothing was added under the capability except the agentic lens and new
           decisions; the other lenses (quality/ux/architecture/run) and the slices
           are untouched.
  - C9     additions are allowed only for `lens/agentic.yaml` and new `decisions/*.yaml`.

Layer rule: reads files on disk only; no git/gh/network.

    python3 check_agentic.py --cap-before <snapshot of capability folder> \
...
def is_agentic_lens(rel):
    parts = rel.split(os.sep)
    return len(parts) >= 2 and parts[-2] == "lens" and parts[-1] == "agentic.yaml"
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="Verify a realize lens persisted result.")
    ...
    # --- capability folder: only agentic lens may change; decisions may be added --
    ...
    for rel, h in after.items():
        if rel in before:
            if h == before[rel]:
                continue
            if is_agentic_lens(rel):
                continue                       # the re-derive is allowed to change
            if is_decision(rel):
                errors.append(f"{rel}: accepted decision edited in place (F9)")
            else:
                errors.append(f"{rel}: changed but only the agentic lens may change (F9)")
        else:
            # newly added file
            if is_agentic_lens(rel) or is_decision(rel):
                continue
            errors.append(f"{rel}: added, but /ux may add only the agentic lens or a decision (F2)")
