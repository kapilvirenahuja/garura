---
name: validate-epic
description: "System-level verification of a deployed product against acceptance criteria, per-milestone. E2E testing via Playwright/Cypress, system-level judge evaluation, manual test scenario generation. Consumes committed code from implement-epic for a specific milestone and locked scenarios from prepare-epic. Produces per-milestone QA verdict: ACCEPT or REJECT. Epic-level ACCEPT requires all milestones ACCEPTED."
user-invocable: true
model: opus
---
...
Given committed code from implement-epic for a specific milestone (feature phase from plan.yaml, e.g. T-001) and locked verification scenarios for that milestone from prepare-epic, deploy to a preview/staging environment and execute system-level verification against that milestone's acceptance criteria. E2E tests run all 3 scenario tiers in order: baseline (cumulative regression from all prior milestones), new (this milestone's scenarios), regression (nothing outside change surface broke). A judge exercises the running product independently via the deployed environment — this is QA acceptance, not developer self-check. When judge failures occur, remediation routes back to implement-epic for code changes for this specific milestone — validate-epic does not invoke code-builder directly. After implement-epic fixes and re-commits, validate-epic re-deploys and re-runs. Max 3 fix iterations. A feature-steward generates manual test scenarios scoped to this milestone. The output is a per-milestone QA verdict: ACCEPT or REJECT. Epic-level ACCEPT requires all milestones individually ACCEPTED.
...
- **judge receives:** Encrypted system-level evals + deployed env URL/credentials. Never builder prompts, unit tests, quality-auditor results from implement-epic, eval-generator prompts, implementation source code (C6).
...
```
/validate-epic --milestone <milestone_id> [--issue <issue-number>] [--deploy-url <url>]

Examples:
  /validate-epic --milestone T-001 --issue 1 --deploy-url https://preview-t001.example.com
  /validate-epic --milestone T-002
```
...
| Read implement-epic status report for this milestone | C1, C18, F1, F14 | Hard halt — implement-epic must have completed this milestone |
...
# C1/C18/F1/F14: Verify implement-epic completed this milestone
...
# Evidence directory
evidence_dir="{stm_dir}/evidence/validate-epic/{milestone_id}"
```
...
- **SE-2 (C1, C18, F1):** implement-epic status report exists at `{milestone_base}/status-report.yaml` showing COMPLETE with all unit tests passing and quality-auditor CERTIFIED.

**Resume check:** If `{stm_dir}/status/validate-epic-{milestone_id}.json` exists, resume — skip completed steps, reset any `in_progress` to pending, continue from first incomplete.
...
4. Hard halt with F1 if implement-epic incomplete, F14 if status report missing
...
Read `{scenarios_yaml_path}` from prepare-epic STM. Build the three tiers for this milestone:
...
Produce structured remediation for implement-epic, scoped to THIS MILESTONE (C8).

**Critical (C8, F4):** Route to implement-epic for THIS MILESTONE — do NOT invoke code-builder directly.
...
## Remediation Instructions for implement-epic (milestone {milestone_id})
...
**Step 8 Eval:**
- **SE-10 (C8, F4):** Remediation file exists. Zero eval IDs, zero eval text. Targets implement-epic for this specific milestone_id.

---

**Step 9 — Route to implement-epic for Milestone Fix**
Owner: orchestrator
Depends on: Step 8

Hand off remediation to implement-epic for THIS MILESTONE. After fixes and re-commit:
...
## Remediation History
| Iteration | Routed to implement-epic | E2E result | Judge result |
|-----------|--------------------------|-----------|--------------|
...
```markdown
# validate-epic Evidence — Milestone {milestone_id}
...
```markdown
## validate-epic complete — Milestone {milestone_id}
...
- **SCE-7 (S7 — Developer):** If fix loop: remediation targets this milestone's implement-epic. Developer can fix without full re-implementation.

- **SCE-8 (S8 — QA Engineer):** validate-epic ran for specific milestone_id. Only milestone-scoped scenarios + cumulative regression executed. Verdict is per-milestone.
...
**Status file:** `{stm_dir}/status/validate-epic-{milestone_id}.json`
...
5. **Judge eval failures:** Fix loop — route to implement-epic for THIS MILESTONE. Max 3 (C9).
...
8. **implement-epic timeout:** Wait. Never invoke code-builder directly (C8, F4).
...
| Field | Value |
|-------|-------|
| intent_hash | sha256:2a38e9ac1565eac0e2fb6420418630ef0fa379a75396ea40c3b6e4c692e20585 |
| compiled_by | /create-play |
| compiled_at | 2026-04-15 |
| workflow_structure | A (conditional fix loop routing to implement-epic per milestone) |
