...
**Pass:** Agent has a documented Context Loading section covering: LTM search at `~/.garura/core/memory/`, sufficiency evaluation, research fallback, and context injection into skill invocations. Agent has the tools needed for research (WebSearch/WebFetch) if its domain requires external knowledge.
