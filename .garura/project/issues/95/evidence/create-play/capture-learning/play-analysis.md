# Play Analysis: capture-learning (Rebake)
...
| 6 | Scenario Validation | play | Yes |
| 7 | Evidence & Close | play | Yes |
...
|------------|---------------|-----|
| C1 (extraction/archival independent) | Play structure | Two independent operations — but extraction is fully bypassed |
...
### Issue 1: L4 Runtime Patterns in High-Order Play
The play uses `intent-resolver` at runtime (Stage 1) with DAG caching. Per ADR 013, high-order plays compile intent at build-time. The intent-resolver is a build-time tool, not a runtime agent. The entire "Load DAG" + "DAG Caching" + "DAG Resumption" infrastructure is L4 overhead.
...
The play uses stages 0-7. L2 compiled format uses named phases (Pre-flight, Preparation/Execution, Checkpoint, Evidence) with sequential numbered steps within each phase.

**Fix:** Restructure into named phases per compiled-example.md format.

### Issue 3: Runtime Intent Reference
The play says "BEFORE executing any step, read reference/intent.yaml". high-order plays do NOT reference intent.yaml at runtime — everything is compiled in.
...
ADR 013 lists capture-learning as "Structure B" (fast execution flow), but the current play has 8 stages including brief + checkpoint. With extraction bypassed, the active flow is: pre-flight → verify issue → archive → validate → evidence. This maps to Structure B.

**Fix:** Compile as Structure B (Pre-flight → Execution → Approval).

### Issue 5: Missing Pause/Resume in L2 Format
The play has DAG-based resumption (L4 pattern). L2 uses per-step status file tracking.
