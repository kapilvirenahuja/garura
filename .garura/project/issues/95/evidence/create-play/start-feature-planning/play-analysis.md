# Play Analysis: start-feature-planning
...
Play: start-feature-planning
  Step 0 — Pre-flight
    Owner: repo-orchestrator (C1), play (C2)
  ...
  Step 4 — Checkpoint (Approval Gate)
    ...
    Note: ONLY approval gate in play
...
4. **No formal step evals** — Play has no step-level eval criteria
5. **Templates reference `feature-started.md`** but file is named `final-report.md` — naming mismatch
6. **Extra frontmatter fields** — `model: sonnet`, `allowed-tools:` are skill fields, not play

## Recommended Workflow

**Structure A — Full checkpoint flow** — Play has preparation work (resolve issue, create branch, deep analysis), a confidence-gated checkpoint (plan approval), and post-checkpoint execution (write evidence, report). Checkpoint is the approval gate where user Tethers or Vanishes.
