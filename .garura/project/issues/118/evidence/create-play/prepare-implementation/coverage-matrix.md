...
## 5. Structural Constraints → Play Structure

Every structural C-n must be enforced by the play's workflow structure.

| C-n | Description | Structural Enforcement | Covered |
|-----|-------------|----------------------|---------|
| C2 | Single invocation per epic — no --phase flag | Play has no --phase argument; single flow from context resolution through lock | YES |
