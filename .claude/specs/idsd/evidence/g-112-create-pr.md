...
**Change:** Moved inline templates to external files per Garura pattern (like start-feature-planning).
...
Step 2 (Checkpoint):
```
Write artifact to STM: `.Garura/{issue-number}/checkpoint/create-pr/{YYYYMMDD-HHMMSS}.md` using `templates/checkpoint.md` with Status: `PENDING_APPROVAL`.
