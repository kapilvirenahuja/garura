...
**Play Version:** 2.0.0 (from play Version table: `Level: L1, Version: 2.0.0`)
**Play File:** `core/components/plays/start-feature/SKILL.md`
...
|---|-----------|-----------|----------|
| 1 | File exists at `core/components/plays/start-feature/SKILL.md` | PASS | File read successfully; 235 lines present |
| 2 | Play declares `Level: L1` and `Agent Calls: 2` | WARN | Version table at bottom declares `Level: L1` and `Distinct Agents: 2`. The field label is "Distinct Agents" not "Agent Calls" — passes in substance but label diverges from gate criterion wording. No explicit "Agent Calls: 2" field in YAML frontmatter. |
| 3 | Play declares `Agents: project-orchestrator, repo-orchestrator` | PASS | Version table row: `Distinct Agents: 2 (project-orchestrator, repo-orchestrator)`. Agents section table also lists both agents. |
| 4 | Play has IDD intent header: `intent`, `constraints`, `failure_conditions` fields | PASS | YAML frontmatter (lines 8–30) contains `intent:`, `constraints:` (list), and `failure_conditions:` (list) — all three present and populated. |
| 5 | Play intent states: "Create or resume a work context — issue + branch + STM directory" | PASS | `intent: > Create or resume a work context — issue + branch + STM directory — as the universal precursor to all tracked work.` — matches exactly (minor extension present, core phrase matches). |
| 6 | Play constraints include: "Must always be the first step for any work" | PASS | `constraints: - Must always be the first step for any work` — exact match on line 15. |
| 7 | Play failure_conditions include: "Branch already exists and has conflicts" | PASS | `failure_conditions: - Branch already exists and has conflicts` — exact match on line 29. |
...
Criterion scope for start-feature: "start-feature play has Tether/Vanish checkpoint after issue + branch creation" and "No play uses AskUserQuestion tool".
...
| Tether → proceed; Vanish → cancel; any other response → clarify | WARN | The play specifies the output text ("Tether to create the branch or Vanish to cancel") but does NOT explicitly describe what to do if the user types something else. No "any other response → clarify" instruction is documented. |
| No use of `AskUserQuestion` tool | PASS | Play `allowed-tools` frontmatter lists: `Task, Read, Write, TaskCreate, TaskUpdate, TaskList`. `AskUserQuestion` is absent. Play text uses output-and-wait phrasing throughout. |
...
| Criterion | Pass/Fail | Evidence |
|-----------|-----------|----------|
| Play identifies three speeds: Fast (minutes), Planned (hours), Strategic (days) | PASS | Final Report section (Contract #6) includes "Next Steps — Choose Your Speed" table with exactly: Fast (Minutes), Planned (Hours), Strategic (Days). |
...
| Strategic speed routes to: full SDLC pipeline starting with `discover-product` | WARN | Final Report shows Strategic → `/discover-product`. The gate criterion says "full SDLC pipeline starting with discover-product" — the play names only the first step, not the full pipeline description. Technically acceptable as a routing hint, but incomplete per gate wording. |
...
| Criterion | Pass/Fail | Evidence |
|-----------|-----------|----------|
| No direct git operations in play | PASS | `allowed-tools` does NOT include `Bash` or `Glob` for git commands. Play explicitly states (line 41): "**Forbidden:** `Bash`, `Grep`, `Glob`, or any direct git/gh commands." |
| No direct issue tracking in play | PASS | Same Forbidden declaration. Issue work is delegated to project-orchestrator via Task tool. |
...
### G-009: Play Level Constraint

| Criterion | Pass/Fail | Evidence |
|-----------|-----------|----------|
| Play is invocable by both Human and Model (atomic play requirement) | PASS | Frontmatter: `user-invocable: true`. Atomic plays are invocable by Human OR Model — no restriction to human-only stated. |
| Play declares ≤2 distinct agents | PASS | Version table: `Distinct Agents: 2 (project-orchestrator, repo-orchestrator)`. Constraint also in YAML: "Maximum 2 distinct agents (project-orchestrator, repo-orchestrator); each may be called multiple times". |
| Play explicitly declares its Level (L1 or L2) in header | WARN | Level is declared in the Version table at the BOTTOM of the file (line 231), not in the YAML frontmatter at the top. The gate criterion says "in its header". The frontmatter contains no `level: L1` field. |
| Play explicitly declares its agent call count | WARN | Agent call count is expressed as "Distinct Agents: 2" in the version table, not as an explicit "Agent Calls" count in the YAML frontmatter. "Distinct Agents" and "Agent Calls" are semantically different (the play notes each agent may be called multiple times). |

---

## Summary

- **Total criteria evaluated:** 27 (17 G-100 + 4 G-003 + 6 G-006 + 4 G-008 + 4 G-009 applied to start-feature, with G-008/G-009 scoped to this play only)
- **Passed:** 21
- **Warned:** 6
- **Failed:** 0
...
| 6 | G-009 | Agent call count declared as "Distinct Agents: 2" not "Agent Calls: 2" — field name mismatch; "distinct" ≠ "calls" (play explicitly notes each agent may be called multiple times, so total calls could exceed 2) |
