...
```
core/components/memory/standards/templates/brief-common.css
```

This file is the single source of truth for all brief styling:
- Token definitions (:root variables)
- Layout (container, header, tabs)
- Components (cards, tables, badges, lists)
- Inline comment system (highlights, popup, comment list)
- Export panel
- Responsive rules

Individual briefs inline this CSS and add zero custom styles.
...
| Tab | Content |
|-----|---------|
| Market Context | problem, target_users, competitors, market_size, differentiators, risks |
| Vision | value_proposition, strategic_goals, success_metrics |
| Scope | assumptions, out_of_scope |
...
| Tab | Content |
|-----|---------|
| Strategy | thesis, narrative |
| Timeline | timeline (horizons + feature refs), exclusions |
| Feasibility | feasibility (per-feature risks), critical_blockers, risk_summary |
| Comments | All inline comments + export |
