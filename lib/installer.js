 ...
 * lib/ is at the repo root, so path.resolve(__dirname, '..') is the package root.
 */
function resolvePackageRoot() {
  const packageRoot = path.resolve(__dirname, '..');
  if (fs.existsSync(path.join(packageRoot, 'core'))) {
    return packageRoot;
  }
  throw new Error('Cannot find core/ directory.');
}
