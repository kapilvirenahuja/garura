...
Used by plays like `prepare` that define a capability graph rather than a fixed step sequence. The play creates the **full task graph upfront** using TaskCreate with `blockedBy` dependencies, then executes capabilities in dependency order.
