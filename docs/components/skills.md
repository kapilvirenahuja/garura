...
| `capture` | true | sonnet | Capture any issue type (feature / bug / defect / epic / enhancement) as a labeled GitHub Issue with background dispatch |
| `report-issue` | true | sonnet | DEPRECATED — alias for `garura:capture`. Use `/capture` instead. |
