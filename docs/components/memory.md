...
The contract is defined in `core/components/memory/standards/kb-extension-conventions.md` and enforced by the `validate-kb-extension` skill. Every feature must carry all nine sections; missing or empty structured sections fail validation and block the pipeline.
...
- **Intent Epic** = generated by `specify-product` per selected capability, conforming to `core/components/memory/standards/intent-epic-schema.yaml`
